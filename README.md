# Melissa Data GeoCoder Object Windows Python3 Sample
...
2.  If you already have the latest DQS Release (ZIP), you can find the data file(s) and dll(s) in there
	...
	- Copy all the dll(s) mentioned above into the `MelissaDataGeoCoderObjectWindowsPython3Sample` project folder.
...
- Interactive 
	...
	```
	$ .\MelissaDataGeoCoderObjectWindowsPython3Sample.ps1
	```
    For quiet mode:
    ```
    $ .\MelissaDataGeoCoderObjectWindowsPython3Sample.ps1 -quiet
    ```
- Command Line 

	You can pass a zip code in ```-zip``` parameter and a license string in ```-license``` parameter to test GeoCoder object. For example:
	```
    $ .\MelissaDataGeoCoderObjectWindowsPython3Sample.ps1 -zip "92688" 
    $ .\MelissaDataGeoCoderObjectWindowsPython3Sample.ps1 -zip "92688" -license "<your_license_string>"
    ```
	For quiet mode:
    ```
    $ .\MelissaDataGeoCoderObjectWindowsPython3Sample.ps1 -zip "92688" -quiet
    $ .\MelissaDataGeoCoderObjectWindowsPython3Sample.ps1 -zip "92688" -license "<your_license_string>" -quiet
    ```
